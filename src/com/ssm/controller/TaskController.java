...
public class TaskController {
	...
	public String taskok(String tidstr, HttpServletRequest request, Model model) {
		...
		taskService.updateTaskEndTime(time,tid);
		User user = null;
		try {
			user = (User) request.getSession(false).getAttribute("nowuser");
		} catch (Exception e) {
	...
	public String addOrder(HttpServletRequest request, Model model) {
        ...
        double reward=5;
		Task task2 = new Task(0, user.getStuid() + "", user.getName(), user.getSchoolid(), 0, reward,
				new Date(), new Date(), "商品:"+product.getName(), "单价:"+product.getPrice(), 0);
